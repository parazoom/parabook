...
class AircraftListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final async = ref.watch(aircraftProvider);
            ...
            return ListView(
              ...
              children: [
                if (active.isNotEmpty) ...[
                  _SubHeader(l.active),
                  ...active.map((ac) => _AircraftTile(ac: ac, l: l, ref: ref)),
                ],
                if (archived.isNotEmpty) ...[
                  _SubHeader(l.archived),
                  ...archived.map((ac) => _AircraftTile(ac: ac, l: l, ref: ref)),
                ],
  ...
  void _showForm(BuildContext context, WidgetRef ref, AppLocalizations l, Aircraft? ac) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      builder: (_) => _AircraftForm(ac: ac, ref: ref, l: l),
    ).then((_) => ref.invalidate(aircraftProvider));
  }
}
...
class _AircraftTile extends StatelessWidget {
  ...
  const _AircraftTile({required this.ac, required this.l, required this.ref});
...
class _AircraftForm extends ConsumerStatefulWidget {
  final Aircraft? ac;
  final WidgetRef ref;
  final AppLocalizations l;

  const _AircraftForm({required this.ac, required this.ref, required this.l});

  @override
  ConsumerState<_AircraftForm> createState() => _AircraftFormState();
}

class _AircraftFormState extends ConsumerState<_AircraftForm> {
  ...
  void initState() {
    super.initState();
    _nameCtrl = TextEditingController(text: widget.ac?.name ?? '');
    _notesCtrl = TextEditingController(text: widget.ac?.notes ?? '');
    _jumpsBeforeCtrl = TextEditingController(text: '${widget.ac?.jumpsBefore ?? 0}');
    _isDefault = widget.ac?.isDefault ?? false;
    _isArchived = widget.ac?.isArchived ?? false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      ...
      child: Column(
        ...
        children: [
          Text(widget.ac != null ? widget.ac!.name : l.addAircraft,
            ...
            if (widget.ac != null)
              TextButton.icon(
                ...
                onPressed: () async {
                  ...
                  if (widget.ac?.id != null) await ref.read(aircraftDaoProvider).delete(widget.ac!.id!);
                  if (mounted) nav.pop();
                },
              ),
            const Spacer(),
  ...
  Future<void> _save() async {
    ...
    if (widget.ac != null) {
      await ref.read(aircraftDaoProvider).update(ac);
    } else {
      await ref.read(aircraftDaoProvider).insert(ac);
    }
